...
### TRF3 case law for Claude, Cursor and AI agents

Search case law from the federal appellate court for São Paulo and Mato Grosso do Sul (TRF3) straight from Claude, ChatGPT or your own agent, covering previdenciário, tributário federal e execução fiscal. Every ruling comes with the judging body, rapporteur, date, the exact snippet that matched your query and the link to the official site. The same connection reaches 16 other Brazilian courts. Free, no login, hosted by the platform.

- ⚖️ **TRF3** plus 16 other Brazilian courts on the same connection
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Jurisprudência TRF3` and **URL** `https://api.mcp.ai/p_trf3`.

### Cursor

[➕ Install Jurisprudência TRF3 in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=trf3&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF90cmYzIn0=)

### VS Code (Copilot Chat)

[➕ Install Jurisprudência TRF3 in VS Code](vscode:mcp/install?name=trf3&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_trf3%22%7D)
...
```
Recent TRF3 rulings on consumer damages
Compare TRF3 and the Superior Court on limitation periods
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_trf3` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
